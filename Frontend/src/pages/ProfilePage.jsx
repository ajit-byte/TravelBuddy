...
export default function ProfilePage() {
  ...
  const handleCoverChange = async (e) => {
    ...
    // Auto-save immediately when picked from the cover camera button
    try {
  ...
  const handleAvatarChange = async (e) => {
    ...
    // Auto-save immediately when picked from the avatar click
    try {
      const url = await uploadImage(file, 'avatars');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/social/profile/me`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'x-auth-token': token() || '' },
        body: JSON.stringify({ profileIconUrl: url }),
      });
      if (res.ok) {
        setAvatarFile(null);
  ...
  // Stage-only handlers used inside the Edit Profile modal (upload happens on Save)
  const handleCoverChangeStage = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > MAX_COVER_MB * 1024 * 1024) {
      setErrors(p => ({ ...p, cover: `Cover image must be under ${MAX_COVER_MB}MB` }));
  ...
  const handleAvatarChangeStage = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > MAX_AVATAR_MB * 1024 * 1024) {
      setErrors(p => ({ ...p, avatar: `Profile photo must be under ${MAX_AVATAR_MB}MB` }));
              ...
              <form onSubmit={handleSave} className="space-y-5">
                ...
                <div className="flex gap-6 items-start pb-5 border-b border-gray-100">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <input ref={editAvatarRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarChangeStage} />
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <input ref={editCoverRef} type="file" accept="image/*" className="hidden" onChange={handleCoverChangeStage} />
                  </div>
                </div>
